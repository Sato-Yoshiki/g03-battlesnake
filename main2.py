...
def move(game_state: typing.Dict) -> typing.Dict:
    ...
    # 敵の体にぶつかるのを防ぐ
    # 頭の上のマスが敵の体（尻尾を除く）ではないか
    if is_move_safe["up"]:
        if my_head_up in enemy_body[:-1]:
            is_move_safe["up"] = False

    # 頭の下のマスが敵の体（尻尾を除く）ではないか
    if is_move_safe["down"]:
        if my_head_down in enemy_body[:-1]:
            is_move_safe["down"] = False

    # 頭の左のマスが敵の体（尻尾を除く）ではないか
    if is_move_safe["left"]:
        if my_head_left in enemy_body[:-1]:
            is_move_safe["left"] = False

    # 頭の右のマスが敵の体（尻尾を除く）ではないか
    if is_move_safe["right"]:
        if my_head_right in enemy_body[:-1]:
            is_move_safe["right"] = False
    ...
    if len(safe_moves) != 0:
        ...
        for i, body in enumerate(enemy_body):
            board[body["x"]][body["y"]] = i + 1
        if "right" in safe_moves:
            if enemy_body[0] in reachable(my_head_right) and len(my_body) <= len(
                enemy_body
            ):
                print("Right:head_to_head_warning!")
        ...
        if "left" in safe_moves:
            if enemy_body[0] in reachable(my_head_left) and len(my_body) <= len(
                enemy_body
            ):
                print("Left:head_to_head_warning!")
            ...
            if counts_left < 20:
                unsafe_moves.append("left")
            if counts_left > max_counts:
                maxDirection = 2
                max_counts = counts_left
        if "up" in safe_moves:
            if enemy_body[0] in reachable(my_head_up) and len(my_body) <= len(
                enemy_body
            ):
                print("Up:head_to_head_warning!")
            ...
            if counts_up < 20:
                unsafe_moves.append("up")
            if counts_up > max_counts:
                maxDirection = 3
                max_counts = counts_up
        if "down" in safe_moves:
            if enemy_body[0] in reachable(my_head_down) and len(my_body) <= len(
                enemy_body
            ):
                print("Down:head_to_head_warning!")
            ...
            if counts_down < 20:
                unsafe_moves.append("down")
            if counts_down > max_counts:
                maxDirection = 4
                max_counts = counts_down
        not_unsafe_moves = []
